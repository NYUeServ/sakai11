...
public class LTIToolHandler implements Handler {
    ...
    private static JwksStore jwks() {
        if (jwksStore.get() == null) {
            // FIXME: default to prod value?
            try {
                jwksStore.set(new JwksStore(new URL(HotReloadConfigurationService.getString("seats.brightspace_jwks_url", "https://nyutest.brightspace.com/d2l/.well-known/jwks"))));
            } catch (Exception e) {
